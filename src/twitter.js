...
class Twitter {
	...
	getUrlObject(url) {
		...
		if (
			displayUrl.startsWith(
				`https://twitter.com/${metadata.username}/status/`
			)
		) {
	...
	async pullTwitterMedia(tweet) {
			...
			for (let url of tweet.entities.urls) {
				if (url.expanded_url.indexOf(`/${tweet.id}/photo/`) > -1) {
					...
					if (
						...
						imgUrls.push(
							`https://v1.opengraph.11ty.dev/${encodeURIComponent(
								targetUrl
							)}/small/`
						);
						medias.push(
		...
		return { mediaHtml: medias, imgUrls };
	}
				...
				if (media.type === "photo") {
					...
					if (media.video_info && media.video_info.variants) {
						...
						try {
							...
							medias.push(
								`<a href="${remoteVideoUrl}">${remoteVideoUrl}</a>`
							);
						}
					}
				}
				...
				${
					!this.isRetweet(tweet)
						? `<span class="tag tag-naked tag-lite${
								!options.showSentiment || sentimentValue === 0
									? " sr-only"
									: ""
						  }">Mood ` +
						  (sentimentValue > 0 ? "+" : "") +
		...
		for (let replyTweet of replies) {
			...
			repliesHtml.push(
				(previousHtml
					...
					tweetHtml +
					(nextHtml
						? `<ol class="tweets-replies">${nextHtml}</ol>`
						: "")
			);
		}
	...
	getMostPopularTweets(tweets, limit = 15, forYear) {
		...
		let topFavorites = topTweets
			.sort(function (a, b) {
				return (
					parseInt(b.favorite_count, 10) -
					parseInt(a.favorite_count, 10)
				);
			})
			.slice(0, limit);
