...
class Index extends Twitter {
	...
	getTopUsersToRetweets(tweets) {
		...
		for (let tweet of tweets) {
			...
			if (
				...
				tweet.entities.user_mentions[0]
			) {
	...
	renderSwearWord(word) {
		return word
			.split("")
			.map((letter, index) => (index === 1 ? "_" : letter))
			.join("");
	}
	...
	getTopSwearWords(tweets = []) {
		...
		tweets
			.filter((tweet) => {
				return (
					!this.isRetweet(tweet) && swearjar.profane(tweet.full_text)
				);
			})
			.forEach((tweet) => {
	...
	async render(data) {
		...
		let last12MonthsTweets = tweets.filter(
			(tweet) =>
				tweet.date - new Date(Date.now() - 1000 * 60 * 60 * 24 * 365) >
				0
		);

		let tweetCount = tweets.length;
		let retweetCount = tweets.filter((tweet) =>
		...
		let mentionNotReplyCount = tweets.filter((tweet) =>
			this.isMention(tweet)
		).length;
		// let ambiguousReplyMentionCount = tweets.filter(tweet => this.isAmbiguousReplyMention(tweet)).length;
		let retweetsEarnedCount = tweets
			.filter((tweet) => !this.isRetweet(tweet))
			.reduce(
				(accumulator, tweet) =>
					accumulator + parseInt(tweet.retweet_count, 10),
				0
			);
		let likesEarnedCount = tweets
			.filter((tweet) => !this.isRetweet(tweet))
			.reduce(
				(accumulator, tweet) =>
					accumulator + parseInt(tweet.favorite_count, 10),
				0
			);

		let topSwears = this.getTopSwearWords(tweets);
		let swearCount = topSwears.reduce(
			(accumulator, obj) => accumulator + obj.count,
			0
		);
		let tweetSwearCount = topSwears.reduce(
			(accumulator, obj) => accumulator + obj.tweets.length,
			0
		);

		let topHashes = this.getTopHashTags(tweets);
		let hashCount = topHashes.reduce(
			(accumulator, obj) => accumulator + obj.count,
			0
		);
		let tweetHashCount = topHashes.reduce(
			(accumulator, obj) => accumulator + obj.tweets.length,
			0
		);
		...
		let mostRecentTweets = tweets
			.filter((tweet) => this.isOriginalPost(tweet))
			.sort(function (a, b) {
				return b.date - a.date;
			})
			.slice(0, 15);
		let recentTweetsHtml = await Promise.all(
			mostRecentTweets.map((tweet) => this.renderTweet(tweet))
		);
		let mostPopularTweetsHtml = await Promise.all(
			this.getMostPopularTweets(tweets)
				.slice(0, 6)
				.map((tweet) =>
					this.renderTweet(tweet, { showPopularity: true })
				)
		);

		let links = this.getAllLinks(tweets);
		let linksCount = links.length;
		let httpsLinksCount = links.filter((entry) =>
		...
		let httpsLinksCount12Months = links12Months.filter((entry) =>
			entry.origin.startsWith("https:")
		).length;
		return `
		<h2 class="tweets-primary-count">
			<span class="tweets-primary-count-num">${this.renderNumber(
				tweetCount
			)}</span> tweet${tweetCount !== 1 ? "s" : ""}
		</h2>
		<!-- <a href="/twitter-search"><h2>Search tweet text</h2></a><hr /> -->
		<div class="js">
			<h2>Search Tweets:</h2>
		...
		<h2 id="retweets">I’ve retweeted other tweets ${this.renderNumber(
			retweetCount
		)} times (${this.renderPercentage(retweetCount, tweetCount)})</h2>
		<div class="lo" style="--lo-stackpoint: 20em">
			<div class="lo-c">
				<h3>Most Retweeted</h3>
				<ol>
					${this.getTopUsersToRetweets(tweets)
						.slice(0, 10)
						.map(
							(user) =>
								`<li><a href="${twitterLink(
									`https://twitter.com/${user.username}`
								)}">${user.username}</a> ${user.count} retweet${
									user.count != 1 ? "s" : ""
								}</li>`
						)
						.join("")}
			...
			<div class="lo-c">
				...
				<ol>
					${this.getTopUsersToRetweets(last12MonthsTweets)
						.slice(0, 10)
						.map(
							(user) =>
								`<li><a href="${twitterLink(
		...
		<h3>${this.renderPercentage(
			replyCount,
			tweetCount
		)} of my tweets are replies (×${this.renderNumber(replyCount)})</h3>
		<div class="lo" style="--lo-stackpoint: 20em">
			<div class="lo-c">
				<h4>Most Replies To</h4>
				<ol>
					${this.getTopReplies(tweets)
						.slice(0, 5)
						.map(
							(user) =>
								`<li><a href="${twitterLink(
									`https://twitter.com/${user.username}`
								)}">${user.username}</a> ${user.count} repl${
									user.count != 1 ? "ies" : "y"
								}</li>`
						)
						.join("")}
			...
			<div class="lo-c">
				...
				<ol>
					${this.getTopReplies(last12MonthsTweets)
						.slice(0, 5)
						.map(
							(user) =>
								`<li><a href="${twitterLink(
		...
		<h3>I’ve sent someone a mention ${this.renderNumber(
			mentionNotReplyCount
		)} times (${this.renderPercentage(
			mentionNotReplyCount,
			tweetCount
		)})</h3>
		...
		<h3>${this.renderPercentage(
			httpsLinksCount,
			linksCount
		)} of the links I’ve posted are using the <code>https:</code> protocol  (${this.renderNumber(
			httpsLinksCount
		)} of ${this.renderNumber(linksCount)})</h3>
		<h3>${this.renderPercentage(
			httpsLinksCount12Months,
			linksCount12Months
		)} of the links I’ve posted in the last 12 months are using the <code>https:</code> protocol  (${this.renderNumber(
			httpsLinksCount12Months
		)} of ${this.renderNumber(linksCount12Months)})</h3>

		<div class="lo" style="--lo-stackpoint: 20em">
			<div class="lo-c">
				<h4>Top Domains</h4>
				<ol>
					${this.getTopDomains(tweets)
						.slice(0, 10)
						.map(
							(entry) =>
								`<li><a href="https://${entry.domain}">${entry.domain}</a> ${entry.count} tweets</li>`
						)
						.join("")}
				</ol>
			</div>
			<div class="lo-c">
				<h4>Top Hosts</h4>
				<ol>
					${this.getTopHosts(tweets)
						.slice(0, 10)
						.map(
							(entry) =>
								`<li><a href="https://${entry.host}">${entry.host}</a> ${entry.count} tweets</li>`
						)
						.join("")}
				</ol>
			</div>
		...
		<h2 id="shared">My tweets have been given about <span class="tag tag-lite tag-retweet">♻️ ${this.renderNumber(
			retweetsEarnedCount
		)}</span> retweets and <span class="tag tag-lite tag-favorite">❤️ ${this.renderNumber(
			likesEarnedCount
		)}</span> likes</h2>
		...
		<ol>
			${emojis
				...
				.map(
					(obj) =>
						`<li>${obj.glyph} used ${obj.count} times on ${obj.tweetcount} tweets</li>`
				)
				.join("")}
		</ol>
		<p><em>${this.renderNumber(emojis.length)} unique emoji on ${this.renderNumber(
			emoji.getTweetCount()
		)} tweets (${this.renderPercentage(
			emoji.getTweetCount(),
			noRetweetsTweetCount
		)} of all tweets***)</em></p>
		...
		<ol>
			${topHashes
				...
				.map(
					(hash) =>
						`<li><code>${hash.tag}</code> used ${
							hash.count
						} times ${
							hash.count > 1 && hash.count > hash.tweets.length
								? `on ${hash.tweets.length} tweet${
										hash.tweets.length !== 1 ? "s" : ""
								  }`
								: ""
						}</li>`
				)
		...
		<p><em>${this.renderNumber(hashCount)} hashtags on ${this.renderNumber(
			tweetHashCount
		)} tweets (${this.renderPercentage(
			tweetHashCount,
			noRetweetsTweetCount
		...
		<ol>
			${topSwears
				...
				.map(
					(swear) =>
						`<li><code>${this.renderSwearWord(
							swear.word
						)}</code> used ${swear.count} times ${
							swear.count > 1 && swear.count > swear.tweets.length
								? `on ${swear.tweets.length} tweet${
										swear.tweets.length !== 1 ? "s" : ""
								  }`
								: ""
						}</li>`
				)
		...
		<p><em>${this.renderNumber(swearCount)} swear words on ${this.renderNumber(
			tweetSwearCount
		)} tweets (${this.renderPercentage(
			tweetSwearCount,
			noRetweetsTweetCount
